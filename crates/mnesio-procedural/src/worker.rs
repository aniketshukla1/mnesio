...
//!
//! Mirrors the [`mnesio_evolve::EvolutionWorker`] in shape — same tail-
...
use mnesio_core::entity::Outcome;
use mnesio_core::event::{Event, LogEntry};
use mnesio_core::types::{ArtifactRef, Id};
...
impl ProceduralWorker {
    ...
    pub async fn try_compile_due(&self) -> Result<usize, MnesioError> {
...
mod tests {
    ...
    use crate::Judge;
    use mnesio_core::entity::{ArtifactKind, JudgeSource, PolicyArtifact};
    use mnesio_core::event::EvalReport;
    use mnesio_core::types::{new_id, BiTemporal, EpisodeRef, ProposalId, Scope, TrajectoryRef};
