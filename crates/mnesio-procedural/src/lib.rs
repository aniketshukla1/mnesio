...
//! [`mnesio_core::Outcome`]s into improved, versioned
//! [`mnesio_core::PolicyArtifact`]s via a GEPA-style (arXiv:2507.19457)
...
//! [`mnesio_core::EvalReport::is_committable`].* This crate enforces it
//! mechanically:
//!
//! - [`mnesio_core::EvalReport::is_committable`] is the non-bypassable
...
impl ProceduralCompiler {
    ...
    /// reflector.
    pub fn new(
        llm: Arc<dyn mnesio_core::LlmClient>,
    ...
    ) -> Result<Option<CompileResult>, MnesioError> {
        // ---- reflect → propose ----
    ...
    pub async fn apply(
        &self,
        log: &dyn mnesio_core::EventLog,
        result: CompileResult,
    ) -> Result<mnesio_core::types::ProposalId, MnesioError> {
        ...
        log.append(mnesio_core::event::Event::ProceduralProposed {
        ...
        match decision {
            ApplyDecision::Commit(report) => {
                log.append(mnesio_core::event::Event::ProceduralCommitted {
                    proposal: proposal_id,
                    report,
                })
                .await?;
            ...
            ApplyDecision::Reject(reason) => {
                log.append(mnesio_core::event::Event::ProceduralRejected {
                    proposal: proposal_id,
                    reason,
                })
                .await?;
...
enum ApplyDecision {
    Commit(mnesio_core::event::EvalReport),
    Reject(String),
}
