...
pub async fn dispatch(
    ...
    match tool_name {
        "mneme_write_memory" => write_memory::handle(ctx, arguments).await,
        "mneme_search" => search::handle(ctx, arguments).await,
        "mneme_record_outcome" => record_outcome::handle(ctx, arguments).await,
