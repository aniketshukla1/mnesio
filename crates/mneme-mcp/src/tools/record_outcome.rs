//! `mneme_record_outcome` — append an `OutcomeRecorded` event.
//!
//! The procedural compiler consumes these. Agents that want their
//! system prompts iteratively improved by mneme should call this
...
use mneme_core::entity::{JudgeSource, Outcome};
use mneme_core::event::Event;
use mneme_core::types::{new_id, ArtifactRef, EpisodeRef, TrajectoryRef};
...
pub fn descriptor() -> ToolDescriptor {
    ToolDescriptor {
        ...
        description: "Record the outcome of an agent task — success/fail plus optional \
                      ...
                      task that used a mneme-managed system prompt.",
        input_schema: json!({
            "type": "object",
            "properties": {
                "episode": {
                    "type": "string",
                    "description": "ULID-style episode identifier — opaque to mneme. If omitted, a \
...
pub async fn handle(ctx: &AppContext, arguments: Value) -> anyhow::Result<CallToolResult> {
    ...
    let episode_id = match args.episode.as_deref() {
        None => new_id(),
        Some(s) => match s.parse::<mneme_core::Id>() {
            Ok(id) => id,
            Err(e) => {
                return Ok(CallToolResult::error_text(format!(
...
mod tests {
    ...
    fn descriptor_schema_is_stable() {
        let d = descriptor();
        assert_eq!(d.name, "mneme_record_outcome");
