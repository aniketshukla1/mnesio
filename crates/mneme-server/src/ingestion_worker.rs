...
//! appended on the fast write path) and runs the `mneme-extract`
//! pipeline on each:
//!
...
impl IngestionWorker {
    ...
    async fn apply_add(
        ...
        let trust = 0.8;
        let importance = mneme_extract::heuristic_importance(&content, &cand_refs, trust, 0.6);
    ...
    async fn publish(&self, event: Event) -> Result<(), MnemeError> {
...
mod tests {
    ...
    impl Retriever for FakeRetriever {
        async fn search(&self, _q: &Query) -> Result<Vec<Hit>, MnemeError> {
            Ok(self
                .hits
                .lock()
                .unwrap()
    ...
    fn temp_dir() -> std::path::PathBuf {
        std::env::temp_dir().join(format!("mneme-ingest-test-{}", new_id()))
    }
