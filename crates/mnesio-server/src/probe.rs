...
//! memory ([`mnesio_probe::falsify`]) so the refuted version is invalidated and
...
pub async fn probe_metrics(
    ...
    if params.apply {
        for o in report.refuted() {
            ...
            let one = vec![mnesio_probe::ProbeOutcome {
                memory: o.memory,
                verdict,
            }];
            match mnesio_probe::falsify(state.log.as_ref(), &scope, &one).await {
                Ok(out) => {
...
fn apply_quiet(res: Result<(), mnesio_core::MnesioError>, view: &str) {
    if let Err(e) = res {
