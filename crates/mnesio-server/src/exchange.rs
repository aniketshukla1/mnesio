...
//!  2. exports it as a signed [`mnesio_exchange::SkillCertificate`] (A's gate
...
use axum::Json;
use mnesio_core::entity::{ArtifactKind, Canary, PolicyArtifact};
use mnesio_core::event::EvalReport;
use mnesio_core::Scope;
#[cfg(not(feature = "ed25519"))]
use mnesio_exchange::FakeSigner;
use mnesio_exchange::{export, import, FakeCanaryRunner, LocalEvaluation};
use mnesio_procedural::EvalGates;
use serde::Serialize;
use std::sync::Arc;
...
#[cfg(not(feature = "ed25519"))]
fn build_signer() -> Box<dyn mnesio_exchange::Signer> {
    Box::new(FakeSigner::new())
}
#[cfg(feature = "ed25519")]
fn build_signer() -> Box<dyn mnesio_exchange::Signer> {
    let s = mnesio_exchange::Ed25519Signer::from_seed([13u8; 32]);
...
pub async fn exchange_metrics(State(state): State<Arc<AppState>>) -> Response {
    ...
    let Some(artifact) = artifacts.into_iter().next() else {
        return Json(ExchangeReport::disabled(
            "no active PolicyArtifact yet — enable the procedural worker (MNESIO_PROCEDURAL=on) \
...
fn summarize(
    res: &Result<mnesio_exchange::ImportOutcome, mnesio_exchange::ImportError>,
) -> (bool, String, u32) {
    use mnesio_exchange::ImportOutcome::*;
    match res {
