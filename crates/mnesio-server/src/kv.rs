...
//!    [`mnesio_core::EvalReport::is_committable`] (Hard Rule #1).
...
use mnesio_core::{EvalReport, Query as RetrievalQuery, Retriever, Scope};
use mnesio_kv::{compile, CartridgeKey, CartridgeStore, FakeKvBackend, KvBackend, SealedMemory};
use mnesio_privacy::Keyring;
use serde::Serialize;
...
pub async fn kv_metrics(State(state): State<Arc<AppState>>) -> Response {
    ...
    #[cfg(feature = "aead")]
    let keyring = Keyring::with_cipher(mnesio_privacy::ChaChaCipher);
    ...
    let payload = KvReport {
        ...
        // is built `--features candle-kv` and `MNESIO_KV_GPU=1` is set.
...
/// Real GPU KV-cartridge demo, behind `--features candle-kv` + `MNESIO_KV_GPU=1`.
/// Loading a model + a CPU baseline is heavy, so it's opt-in and the result is
/// computed once and cached for the process lifetime.
///
/// Configurable via env:
/// - `MNESIO_KV_GPU_MODEL` — HF repo (default `Qwen/Qwen2.5-0.5B-Instruct`); any
///   Qwen2-family size loads (e.g. `Qwen/Qwen2.5-1.5B-Instruct`).
/// - `MNESIO_KV_GPU_PRECISION` — `f32` | `f16` | `bf16` (default `f32`). Deep
///   models need `bf16`, not `f16` (f16's narrow exponent overflows).
/// - `MNESIO_KV_GPU_CPU` — `0` skips the CPU baseline (the speedup is omitted);
///   useful for large models where a CPU prefill would be very slow.
#[cfg(feature = "candle-kv")]
mod gpu {
    use super::GpuKvReport;
    use mnesio_kv::{CandlePrecision, KvBackend, QwenCandleBackend, CANDLE_DEFAULT_REPO};
    ...
    fn model_repo() -> String {
        std::env::var("MNESIO_KV_GPU_MODEL").unwrap_or_else(|_| CANDLE_DEFAULT_REPO.to_string())
    }
    fn precision() -> CandlePrecision {
        std::env::var("MNESIO_KV_GPU_PRECISION")
            .ok()
            .and_then(|s| CandlePrecision::from_label(&s))
            .unwrap_or(CandlePrecision::F32)
    }
    fn cpu_baseline_enabled() -> bool {
        std::env::var("MNESIO_KV_GPU_CPU").ok().as_deref() != Some("0")
    }

    pub async fn summary() -> Option<GpuKvReport> {
        if std::env::var("MNESIO_KV_GPU").ok().as_deref() != Some("1") {
            return Some(GpuKvReport::disabled(
                "set MNESIO_KV_GPU=1 to load the real GPU KV backend (downloads the model on \
                 first run, then caches). Pick model/precision with MNESIO_KV_GPU_MODEL + \
                 MNESIO_KV_GPU_PRECISION (f32|f16|bf16)"
                    .to_string(),
            ));
        }
