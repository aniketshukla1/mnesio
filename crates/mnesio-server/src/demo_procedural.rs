...
//!    [`mnesio_procedural::ProceduralStore`] has something to track.
...
use mnesio_core::entity::{ArtifactKind, Canary, JudgeSource, Outcome, PolicyArtifact};
use mnesio_core::event::{EvalReport, Event};
...
/// reference it.
pub async fn seed_initial_artifact(log: &Arc<dyn EventLog>) -> Result<PolicyArtifact, MnesioError> {
    let a = seed_artifact();
...
/// success/score mixes.
pub fn spawn_outcomes(log: Arc<dyn EventLog>, artifact_id: mnesio_core::Id) {
...
pub fn build_compiler(llm: Arc<dyn LlmClient>, min_batch: usize) -> Arc<ProceduralCompiler> {
    ...
    c.trigger = mnesio_procedural::CompileTrigger {
        min_batch,
...
) -> Result<PolicyArtifact, MnesioError> {
