...
use mneme_core::entity::{ArtifactKind, JudgeSource, Outcome, PolicyArtifact};
...
) -> Result<mneme_procedural::SuiteReport> {
...
pub struct ComparisonReport {
    ...
    pub report_a: mneme_procedural::SuiteReport,
    pub report_b: mneme_procedural::SuiteReport,
...
pub async fn compare_artifacts(
    ...
    for r in &report_a.task_results {
        if r.kind == mneme_procedural::TaskKind::Benchmark {
    ...
    for r in &report_b.task_results {
        if r.kind == mneme_procedural::TaskKind::Benchmark && r.passed {
...
pub async fn run_bench(
    ...
    for _pass in 1..max_versions {
        ...
        let Some(result) = compiler
            .compile_with_inputs(&current, &outcomes, &shadow_inputs, "mneme-bench")
            .await?
        else {
...
async fn emit_curve_point(
    ...
    version: u32,
    report: &mneme_procedural::SuiteReport,
    objective_delta: f32,
    judges_consulted: u8,
) -> Result<()> {
...
fn passing_baseline_report() -> mneme_core::event::EvalReport {
    mneme_core::event::EvalReport {
        canaries_passed: 0,
        canaries_total: 0,
...
impl EventLog for MemoryLog {
    async fn append(&self, event: Event) -> Result<Id, MnemeError> {
        ...
        self.entries
            .lock()
            .map_err(|e| MnemeError::Other(anyhow!("log mutex poisoned: {e}")))?
...
/// progression `mneme-server/src/demo_llm.rs` uses, vendored here so
