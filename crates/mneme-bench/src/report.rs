...
use mneme_procedural::{LearningCurvePoint, TaskKind};
...
/// Render a learning-curve report — what `mneme-bench run` produces.
pub fn render_learning_curve(
    ...
    template(
        &format!("mneme-bench · {suite_name} · learning curve"),
        &format!(
            r##"
...
/// Render an A/B comparison report — what `mneme-bench compare` produces.
pub fn render_comparison(report: &ComparisonReport) -> String {
    ...
    template(
        &format!("mneme-bench · {} · A vs B", report.suite_name),
        &format!(
            r##"
...
{body}
<div class="footer">
  Generated by <code>mneme-bench</code>. The procedural compiler's commit gate
  ...
  <code>mneme_procedural::gate</code>.
</div>
</body>
</html>"##
    )
...
fn comparison_drill_table(report: &ComparisonReport) -> String {
    ...
    let mut b_by_input: std::collections::HashMap<&str, &mneme_procedural::TaskResult> =
...
mod tests {
    use super::*;
    use mneme_core::types::{new_id, ArtifactRef};
    ...
    fn comparison_html_uses_grouped_bars_and_drill_table() {
        ...
        let report = ComparisonReport {
            ...
            report_a: mneme_procedural::SuiteReport {
                benchmark_score: 0.4,
                benchmark_passed: 2,
                benchmark_total: 5,
                safety_probe_pass_rate: 1.0,
            ...
            report_b: mneme_procedural::SuiteReport {
                benchmark_score: 0.8,
                benchmark_passed: 4,
                benchmark_total: 5,
                safety_probe_pass_rate: 1.0,
