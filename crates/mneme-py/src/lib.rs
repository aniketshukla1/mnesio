//! # mneme — Python bindings
//!
//! Pip-installable extension exposing mneme's core APIs to Python.
...
impl SearchResult {
    fn __repr__(&self) -> String {
        format!(
            "<mneme.SearchResult hits={} citations={}>",
            self.hits.len(),
            self.citations.len()
        )
    }
...
impl SearchHit {
    fn __repr__(&self) -> String {
        format!(
            "<mneme.SearchHit memory_id={} score={:.3}>",
            self.memory_id, self.score
        )
    }
}

/// The `mneme._mneme` extension module that the Python package
/// re-exports under `mneme.*`.
#[pymodule]
fn _mneme(py: Python<'_>, m: &Bound<'_, PyModule>) -> PyResult<()> {
    m.add_class::<Client>()?;
    m.add_class::<SearchResult>()?;
    m.add_class::<SearchHit>()?;

    // Expose a `MnemeError` alias for `RuntimeError` so callers can
    // `except mneme.MnemeError:` if they want a single catch-all.
    let runtime_error = py.get_type_bound::<PyRuntimeError>();
    m.add("MnemeError", runtime_error)?;
