//! `mnesio_write_memory` — append a new memory.
//!
...
pub fn descriptor() -> ToolDescriptor {
    ToolDescriptor {
        ...
        description: "Append a new memory to mnesio. The content is embedded synchronously, \
...
pub async fn handle(ctx: &AppContext, arguments: Value) -> anyhow::Result<CallToolResult> {
    ...
    ctx.vector
        .apply(&mnesio_core::event::LogEntry {
            id: id1,
            event: written,
        })
        .await?;
...
mod tests {
    ...
    fn descriptor_schema_is_stable() {
        let d = descriptor();
        assert_eq!(d.name, "mnesio_write_memory");
