//! `mnesio_search` — hybrid retrieval + synthesized answer.
//!
...
pub fn descriptor() -> ToolDescriptor {
    ToolDescriptor {
        name: "mnesio_search",
        description: "Search mnesio's memory store. Returns a synthesized answer composed of \
...
pub async fn handle(ctx: &AppContext, arguments: Value) -> anyhow::Result<CallToolResult> {
    ...
    let mut contents: HashMap<mnesio_core::Id, (String, Vec<String>)> = HashMap::new();
    for entry in &entries {
        if let mnesio_core::event::Event::MemoryWritten(m) = &entry.event {
...
mod tests {
    ...
    fn descriptor_schema_is_stable() {
        let d = descriptor();
        assert_eq!(d.name, "mnesio_search");
