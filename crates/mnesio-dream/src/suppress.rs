...
use mnesio_procedural::{EvalGates, RejectReason};
...
pub trait SuppressionEvaluator: Send + Sync {
    async fn evaluate(&self, rule: &SuppressionRule) -> Result<EvalReport, MnesioError>;
}
...
impl SuppressionLearner {
    ...
    ) -> Result<Vec<SuppressionOutcome>, MnesioError> {
