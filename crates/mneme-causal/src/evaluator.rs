...
//!
//! Depends only on `mneme-core` (the `Retriever` trait), so the production
//! `HybridRetriever` drops in as `Arc<dyn Retriever>` and tests use a tiny
//! fake — no `mneme-index` coupling (Hard Rule #7).

use crate::contribution::CounterfactualEvaluator;
use async_trait::async_trait;
use mneme_core::traits::{Query, Retriever};
