...
//! wire format. That single shape reaches three things mneme cares about:
//!
...
//! `OPENROUTER_API_KEY` → `OPENAI_API_KEY` → `MNEME_OPENAI_API_KEY`.
//!
//! Overrides (all env, with sensible defaults):
//!
//! - `MNEME_OPENAI_BASE_URL` — default [`DEFAULT_OPENAI_BASE_URL`] (OpenRouter).
//! - `MNEME_OPENAI_MODEL`    — default [`DEFAULT_OPENAI_MODEL`].
//!
...
/// `MNEME_OPENAI_MODEL` (e.g. `anthropic/claude-3.5-sonnet` for a
...
impl OpenAiCompatClient {
    ...
    /// `OPENROUTER_API_KEY` → `OPENAI_API_KEY` → `MNEME_OPENAI_API_KEY`;
    /// base URL and model from `MNEME_OPENAI_BASE_URL` /
    /// `MNEME_OPENAI_MODEL` (with defaults). Returns an actionable error
    /// if no key is set — the one thing that can't have a default.
    pub fn from_env() -> Result<Self, MnemeError> {
        let api_key = std::env::var("OPENROUTER_API_KEY")
            .or_else(|_| std::env::var("OPENAI_API_KEY"))
            .or_else(|_| std::env::var("MNEME_OPENAI_API_KEY"))
            .map_err(|_| {
                MnemeError::Llm(
                    "no API key found: set OPENROUTER_API_KEY (or OPENAI_API_KEY / \
                     MNEME_OPENAI_API_KEY) in the environment. The key is never \
                     read from a file or flag."
                        .into(),
                )
            })?;
        let base_url = std::env::var("MNEME_OPENAI_BASE_URL")
            .unwrap_or_else(|_| DEFAULT_OPENAI_BASE_URL.into());
        let model =
            std::env::var("MNEME_OPENAI_MODEL").unwrap_or_else(|_| DEFAULT_OPENAI_MODEL.into());
...
impl LlmClient for OpenAiCompatClient {
    async fn complete(&self, prompt: &str) -> Result<String, MnemeError> {
        ...
        let resp = self
            ...
            .send()
            .await
            .map_err(|e| MnemeError::Llm(format!("openai-compat POST {url}: {e}")))?;
        ...
        let body: ChatResponse = resp
            .json()
            .await
            .map_err(|e| MnemeError::Llm(format!("openai-compat JSON decode: {e}")))?;
        body.choices
            .into_iter()
            .next()
            .map(|c| c.message.content)
            .ok_or_else(|| MnemeError::Llm("openai-compat: response had no choices".into()))
    }
}
