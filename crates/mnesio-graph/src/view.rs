...
use mnesio_core::entity::{Memory, Source};
use mnesio_core::event::{Event, LogEntry};
use mnesio_core::traits::MaterializedView;
use mnesio_core::types::{Id, MemoryRef, Scope};
...
impl FjallGraphView {
    ...
    fn put_node(&self, n: &NodeRecord) -> Result<(), MnesioError> {
        let key = encode_node_key(n.id);
        let bytes = bincode::serialize(n).map_err(|e| MnesioError::Storage(e.to_string()))?;
        self.nodes
            .insert(key, bytes)
            .map_err(|e| MnesioError::Storage(e.to_string()))?;
        Ok(())
    ...
    fn get_node_raw(&self, id: MemoryRef) -> Result<Option<NodeRecord>, MnesioError> {
    ...
    fn put_edge(&self, e: &EdgeRecord) -> Result<(), MnesioError> {
        ...
        let bytes = bincode::serialize(e).map_err(|e| MnesioError::Storage(e.to_string()))?;
        self.edges_out
            .insert(k_out, bytes.clone())
            .map_err(|err| MnesioError::Storage(err.to_string()))?;
        self.edges_in
            .insert(k_in, bytes)
            .map_err(|err| MnesioError::Storage(err.to_string()))?;
        Ok(())
    ...
    ) -> Result<Vec<EdgeRecord>, MnesioError> {
    ...
    fn persist(&self) -> Result<(), MnesioError> {
        self.keyspace
            .persist(fjall::PersistMode::Buffer)
            .map_err(|e| MnesioError::Storage(e.to_string()))
    }

    fn record_checkpoint(&self, id: Id) -> Result<(), MnesioError> {
        self.meta
    ...
    ) -> Result<Option<NodeRecord>, MnesioError> {
    ...
    pub fn roots(
        ...
        as_of: Option<OffsetDateTime>,
        limit: usize,
    ) -> Result<Vec<NodeDegree>, MnesioError> {
    ...
    pub fn stats(&self) -> Result<GraphStats, MnesioError> {
...
impl MaterializedView for FjallGraphView {
    ...
    async fn checkpoint(&self) -> Result<Option<Id>, MnesioError> {
        ...
        if v.len() != 16 {
            return Err(MnesioError::Storage(format!(
