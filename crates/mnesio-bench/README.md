...
Eval-as-product harness for mnesio. Two jobs in one crate:
...
cargo run -p mnesio-bench -- run --suite gsm8k --max-versions 6

# memory recall@k over a built-in suite (offline, mock embedder)
cargo run -p mnesio-bench -- memeval --suite locomo --k 10 --embedder mock

# large-scale load test: throughput + latency percentiles + recall
cargo run -p mnesio-bench --release -- scale --sizes 1000,10000,50000 --embedder mock
...
`mnesio_index::rerank::LexicalReranker` as the retriever's final stage;
...
```bash
cargo run -p mnesio-bench --release -- memeval \
...
real path, **separating the two write phases** so the numbers reflect mnesio's
architecture:
...
Runs mnesio's *measured* retrieval recall@k on the LOCOMO/LongMemEval mini-suites
...
All numbers below are **measured**, not projected — `mnesio-bench` on a 2021
...
```bash
cargo test -p mnesio-bench                  # 24 tests (offline, no network)
cargo test -p mnesio-bench --features fetch # 31 tests (+7 real-data loaders)
```
...
- Downloaded suites cache to `crates/mnesio-bench/data/cache/` (gitignored).
  Override with `MNESIO_BENCH_CACHE=/path`.
