//! Competitive comparison harness — mnesio in context, honestly.
//!
...
//!    mnesio's moat lives (append-only + replayable + bi-temporal substrate
...
//!    LOCOMO / LongMemEval. mnesio's offline, LLM-free number is **retrieval
...
//!    A true apples-to-apples QA-J run needs an LLM judge over mnesio's
//!    retrieved context (mnesio has the `Judge` + synthesizer path for it; it's
...
/// A published benchmark number, with its exact source. **Not** mnesio's — these
...
/// end-to-end answer-correctness metrics — distinct from mnesio's retrieval
...
/// A full competitive report: mnesio's *measured* retrieval recall on the
/// LOCOMO/LongMemEval-style suites, plus the capability matrix and the cited
/// competitor QA scores.
pub struct CompeteReport {
    pub k: usize,
    pub embedder: String,
    pub mnesio_locomo: MemEvalReport,
    pub mnesio_longmemeval: MemEvalReport,
    pub capabilities: Vec<CapabilityRow>,
    pub cited: Vec<CitedResult>,
}

/// Run mnesio's own measured numbers and assemble the comparison.
pub async fn run_compete(k: usize, embedder: &str) -> Result<CompeteReport> {
    ...
    let mnesio_locomo = run_memeval(&locomo, k, embedder, false).await?;
    let mnesio_longmemeval = run_memeval(&longmemeval, k, embedder, false).await?;
    Ok(CompeteReport {
        k,
...
pub fn compete_markdown(r: &CompeteReport) -> String {
    ...
    out.push_str("# How mnesio compares\n\n");
    out.push_str(
         ...
         **end-to-end QA accuracy** with mnesio's *measured* **retrieval \
    ...
    out.push_str("| Capability | mnesio | Mem0 | Zep | Letta | A-MEM |\n");
    out.push_str("|---|:---:|:---:|:---:|:---:|:---:|\n");
    ...
    out.push_str(
        "\n✅ shipped · ◑ partial · — not in published design. Competitor cells \
         reflect each system's published architecture and may evolve. mnesio is \
    ...
    // --- mnesio measured ---
    out.push_str(&format!(
        "## mnesio — measured retrieval recall@{} (embedder: {})\n\n",
        r.k, r.embedder
    ));
    out.push_str("| Suite | Memories | Questions | recall@k |\n");
    out.push_str("|---|---:|---:|---:|\n");
    for (label, rep) in [
        ("LOCOMO-mini", &r.mnesio_locomo),
        ("LongMemEval-mini", &r.mnesio_longmemeval),
    ] {
        out.push_str(&format!(
            "| {} | {} | {} | {:.1}% |\n",
            label,
    ...
    out.push_str(
        "\nMeasured live through the real `FjallEventLog → VectorView + Bm25View \
         ...
         for a published end-to-end number, run `mnesio-bench fetch` (SQuAD) or \
    ...
    out.push_str(
        "\nThese are competitor/baseline numbers transcribed from the cited \
         papers — **not** mnesio's, and **not** directly comparable to the \
         recall@k above (different metric). They establish the landscape mnesio \
         enters; mnesio's differentiation is the capability matrix.\n",
    );
...
mod tests {
    ...
    #[test]
    fn capability_matrix_is_complete_and_mnesio_leads_the_moat_rows() {
        let m = capability_matrix();
        assert!(m.len() >= 10, "expected a substantial matrix");
        // Every row must have mnesio support (Yes/Partial), never No.
        for row in &m {
            assert_ne!(
                row.mnesio,
                Support::No,
                "mnesio should support every listed capability: {}",
                row.capability
            );
        }
        // The frontier moat rows should be mnesio-only (no competitor Yes).
        let moat = m
            .iter()
            .find(|r| r.capability.contains("Non-bypassable commit gate"))
            .unwrap();
        assert_eq!(moat.mnesio, Support::Yes);
    ...
    async fn run_compete_produces_measured_recall_and_markdown() {
        ...
        assert!(r.mnesio_locomo.total_questions > 0);
        assert!(r.mnesio_longmemeval.total_questions > 0);
