...
use mnesio_core::types::{new_id, BiTemporal, Id, MemoryRef, Scope};
use mnesio_core::{Embedder, EventLog, Query, Retriever};
use mnesio_index::{Bm25View, HybridRetriever, MockEmbedder, VectorView};
...
impl Env {
    ...
    fn open_with_capacity(capacity: usize) -> Result<Self> {
        let dir = std::env::temp_dir().join(format!("mnesio-edge-{}", new_id()));
