# Competitive analysis — how mnesio becomes the best memory layer

_Last updated: 2026-05. Sources are linked at the bottom._

## TL;DR

mnesio's **wedge is unique**: nobody else combines (1) self-improving
...
is the layer past parity: six things only mnesio's substrate can ship safely.

## Where mnesio already stands

| Capability | Field leader | mnesio |
|---|---|---|
...
### P0 — table stakes
1. **Ingestion extraction + consolidation** (`mnesio-extract`): raw turn →
...
3. **LOCOMO + LongMemEval harnesses** in `mnesio-bench` + published numbers.
...
possible — or only *safe* — because mnesio's system of record is an append-only,
...
Thesis: **mnesio is not a place to put facts — it's a memory that gets
