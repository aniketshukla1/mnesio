...
We score every competitor on **mnesio's seven moat dimensions**:
...
|---|:--:|:--:|:--:|:--:|:--:|:--:|:--:|---|:--:|
| **mnesio** | ✅ | ✅ | ✅ | ✅ | ✅ | ✅(gated) | ✅ | LoCoMo **not yet published** (98.1% SQuAD recall) | — |
...
**mnesio is the only row that's filled across all seven.** GATE and SHRED are
unmatched by *every* competitor; KV is unmatched as a *governed* view. The honest
hole is REPRO+headline: mnesio has the only reproducible-by-design harness but **no
...
- **vs mnesio — they win:** distribution, ecosystem, a *published* (if disputed) number, funding.
- **vs mnesio — we win:** no gate, treats user change as *replacement not evolution* (their own admitted gap), no append-only/bi-temporal, no provenance/crypto-shred, no gated procedural, no KV. License Apache-2.0 but advanced features paywalled; mnesio is fully open.
...
- **vs mnesio — they win:** mature temporal-graph retrieval, strong on facts-that-change, real adoption.
- **vs mnesio — we win:** no gate, no append-only event log (graph is the store, not a replayable view), no KV, no crypto-shred, no procedural; **open-core is closing** (CE deprecated) → mnesio's fully-OSS + self-host story is a wedge. atlan explicitly notes Zep has "no constitutional governance layer."
...
- **vs mnesio — they win:** best reproduced accuracy; richer multi-strategy retrieval than mnesio's RRF; MIT + self-host.
- **vs mnesio — we win:** no gate, no append-only/bi-temporal substrate, no KV, no crypto-shred, no gated procedural. Their "reflect" ≈ mnesio dreaming but ungated. **Borrow their 4-strategy+rerank retrieval; beat them on governance + verifiable improvement.**
...
- **vs mnesio — they win:** mainstreamed the self-improvement/skill-reuse *narrative*; distribution (plugin, MemTensor backing, paper); explicit token-savings number.
- **vs mnesio — we win:** **no safety/eval gate before committing memory changes** (their docs show none), not append-only (delete API, no immutable history/versioning), not KV-cache, no provenance/time-travel/crypto-shred. **This is the key reframe: MemOS = self-improving; mnesio = *safely* self-improving + governed.**
...
- **vs mnesio — they win:** agent-as-its-own-memory runtime, strong mindshare/lineage.
- **vs mnesio — we win:** self-editing is ungated (no canary/safety gate), no bi-temporal log, no KV/provenance/crypto-shred. mnesio is a *memory layer* you bolt on; Letta is a *runtime* — partly complementary.
...
- **vs mnesio — they win:** fastest dev setup, MCP-native polish, RAG bundled.
- **vs mnesio — we win:** closed-source (mnesio fully OSS), no gate/append-only/KV/provenance, "forget" is a delete not crypto-shred-on-immutable-log, "compliance posture unestablished" (per atlan).
...
- **vs mnesio — they win:** top LoCoMo, best multi-hop/temporal retrieval, coding-agent fit.
- **vs mnesio — we win:** none of GATE/BITEMP/KV/PROV/SHRED/PROC (per their own post). **Adopt the Context-Tree idea into mnesio retrieval.**
...
- **vs mnesio — they win:** deep user modeling, token efficiency, a real "dream" loop shipped.
- **vs mnesio — we win:** no gate/append-only/KV/provenance/crypto-shred; dream is ungated. mnesio's dreaming is gated + prunes-by-contribution.
...
- **vs mnesio — they win:** strong accuracy, verbatim fidelity, recent paper.
- **vs mnesio — we win:** verbatim-everything doesn't scale-govern (no consolidation/decay gate, no KV amortization, no provenance/crypto-shred, no procedural).

### Memori — 🟡 "memory as data with schema + history"
- **Architecture:** structured, queryable, schema'd memory from agent traces; positions on trustworthiness/history. Closest to mnesio's *positioning*.
- **vs mnesio — they win:** clean schema-first story, agent-trace ingestion.
- **vs mnesio — we win:** "history" ≠ append-only+bi-temporal+invalidate-supersede; no gate/KV/crypto-shred/gated-procedural; no reproducible eval shown.
...
- **LangMem:** LangGraph-coupled KV+vector; notably **"procedural memory — agents update their own system prompts"** (ungated) — the direct conceptual overlap with mnesio's compiler, minus the gate. MIT, Python-only.
...
- **vs mnesio — they win:** ByteDance backing + distribution; **hierarchical context delivery + skills** is the single closest system to *both* mnesio's wedge *and* the Context-Tree retrieval idea we're roadmapping; zero-infra.
- **vs mnesio — we win:** no gate; filesystem "self-evolving" ≠ append-only + invalidate-and-supersede on an immutable log; no KV cartridge, no crypto-shred, no reproducible eval; skill reuse is **ungated**.
...
- **vs mnesio — they win:** production polish; its retrieval stack is **exactly the multi-signal + reranker** parity work below; low overhead (no LLM extraction on write).
- **vs mnesio — we win:** closed/commercial (mnesio fully OSS); no gate/append-only/KV/provenance/crypto-shred shown; "invalidation" without an immutable-log substrate.
...
- **vs mnesio — they win:** companion UX, ecosystem reach, strong self-reported LoCoMo, decay/forgetting shipped.
- **vs mnesio — we win:** no gate/append-only/KV/provenance/crypto-shred; "forgetting" is decay/deprioritize, not crypto-shred on an immutable log; no reproducible methodology. Different buyer (companions).
...
- **vs mnesio — they win:** the most complete memory **taxonomy** (incl. a Procedural type), multimodal/screenshot capture, local privacy.
- **vs mnesio — we win:** MIRIX's "Procedural" is a *storage category*, not a **gated self-improvement loop**; no gate/append-only/KV/crypto-shred/provenance. (Good system to cite when explaining mnesio's procedural is *gated*, not just a bucket.)
...
- **vs mnesio — they win:** privacy-by-design (local-only), personal-identity framing.
- **vs mnesio — we win:** it's a *personal AI identity* product, not agent-memory infra — no gate/append-only/KV/provenance/crypto-shred/procedural, not a benchmark player. Mostly a different market than mnesio.
...
| Decay/forgetting | PowerMem (Ebbinghaus), Vestige (FSRS-6), Suyi (dual-temporal), widemem-ai | nibble at consolidation | heuristic decay vs mnesio's causal/contribution GC |
...
| Multimodal/video memory | m3-agent, HippoMM, WorldMM, MemVerse, Visual Agentic Memory | adjacent market | out of mnesio's current scope; MIRIX profiled above |

## The throughline — how mnesio beats the field
...
2. **atlan's verdict — "no framework provides enterprise governance / lineage / policy compliance" — is mnesio's lane.** Own governed/verifiable/regulator-grade memory; the ~90 others fight over recall.
...
4. **Absorb two ideas:** Hindsight's 4-strategy+cross-encoder retrieval and ByteRover's Context-Tree → mnesio's RRF.
